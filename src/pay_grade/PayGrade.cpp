...
void PayGrade::handleAddPaygrade() {
    ...
    float grade_basic, grade_da, grade_ta, grade_bonus;
    ...
    addPaygradeToDatabase(department_name, grade_name, grade_basic, grade_da, grade_ta, grade_bonus);
}
...
void PayGrade::addPaygradeToDatabase(const std::string &department_name, const std::string &grade_name,
    ...
    const char* sql = "INSERT INTO PayGrade (department_name, grade_name, grade_basic, grade_da, grade_ta, grade_bonus) "
                      "VALUES (?, ?, ?, ?, ?, ?);";
    ...
    sqlite3_bind_text(stmt, 1, department_name.c_str(), -1, SQLITE_STATIC);
    sqlite3_bind_text(stmt, 2, grade_name.c_str(), -1, SQLITE_STATIC);
    sqlite3_bind_double(stmt, 3, grade_basic);
    sqlite3_bind_double(stmt, 4, grade_da);
    sqlite3_bind_double(stmt, 5, grade_ta);
    sqlite3_bind_double(stmt, 6, grade_bonus);
...
std::vector<PayGradeDetail> PayGrade::listPayGradesByDepartment(const std::string& department) {
    ...
    std::string sql;
    if (department == "all") {
        sql = "SELECT department_name, grade_name, grade_basic, grade_da, grade_ta, grade_bonus FROM PayGrade ORDER BY department_name;";
    } else {
        sql = "SELECT grade_name, grade_basic, grade_da, grade_ta, grade_bonus FROM PayGrade WHERE department_name = '" + department + "';";
    }
...
void PayGrade::printPayGrades(const std::vector<PayGradeDetail>& payGrades) {
    ...
    std::cout << std::left << std::setw(20) << "Department" << std::setw(15) << "Grade Name" << std::setw(10) << "Basic"
              << std::setw(10) << "DA" << std::setw(10) << "TA" << std::setw(10) << "Bonus" << std::endl;
    ...
    for (const auto& detail : payGrades) {
        std::cout << std::left << std::setw(20) << detail.department_name << std::setw(15) << detail.grade_name 
                  ...
                  << std::setw(10) << detail.travel_allowance << std::setw(10) << detail.bonus << std::endl;
    }
}
...
void PayGrade::collectUpdateInputs(std::string& gradeToUpdate, float& basic, float& da, float& ta, float& bonus) {
...
void PayGrade::updatePayGradeDetails(const std::string& gradeToUpdate, float basic, float da, float ta, float bonus) {
    ...
    std::string sql = "UPDATE PayGrade SET grade_basic = ?, grade_da = ?, grade_ta = ?, grade_bonus = ? WHERE grade_name = ?";
    sqlite3_stmt* stmt;
    ...
    sqlite3_bind_text(stmt, 5, gradeToUpdate.c_str(), -1, SQLITE_STATIC);
